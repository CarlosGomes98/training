...
def inference(
    ...
    results = []
    with torch.no_grad():
        for i in tqdm(range(0, len(prompts), bs), desc="Generating images", ):
...
def main():
    ...
    if prompts:
        ...
        if padding_size > 0:
            padding_tensor = torch.zeros(padding_size, *images.shape[1:], device=trainer.device, dtype=images.dtype)
        ...
        images = torch.zeros(math.ceil(total_prompts / world_size), 3, img_size, img_size, device=trainer.device, dtype=trainer._dtype)
