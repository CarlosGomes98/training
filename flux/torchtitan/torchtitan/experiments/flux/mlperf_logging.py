...
class MLPerfLogger:
    ...
    def log_run_start(
        ...
        self.mllogger.event(key=constants.OPT_ADAMW_BETA_1, value=optimizer_config["betas"][0])
        self.mllogger.event(key=constants.OPT_ADAMW_BETA_2, value=optimizer_config["betas"][1])
        self.mllogger.event(key=constants.OPT_ADAMW_EPSILON, value=optimizer_config["eps"])
        self.mllogger.event(key=constants.OPT_ADAMW_WEIGHT_DECAY, value=optimizer_config["weight_decay"])
        self.mllogger.event(key=constants.OPT_BASE_LR, value=lr)
        self.mllogger.event(key=constants.OPT_GRADIENT_CLIP_NORM, value=gradient_clip_norm)
    ...
    def log_train_step_start(self, step: int):
        if step % self.log_every_n_steps != 1:
            return
        self.mllogger.start(
    ...
    def log_train_step_end(
        ...
        self.mllogger.end(
            ...
            value="training_step",
            metadata={constants.SAMPLES_COUNT: step * self.gbs - self.log_every_n_steps * self.gbs},
        )
        self.mllogger.event(
            key="tracked_stats",
            value={
            ...
            metadata={constants.STEP_NUM: step - self.log_every_n_steps},
        )

    def log_eval_start(self, step: int):
        self.mllogger.event(
            key=constants.EVAL_START, metadata={constants.SAMPLES_COUNT: step * self.gbs}
        )
    ...
    def log_eval_end(self, step: int, loss: float):
        ...
        self.mllogger.end(
            key=constants.EVAL_STOP, metadata={constants.SAMPLES_COUNT: step * self.gbs}
        )
