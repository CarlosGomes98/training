...
df['file_name'] = df['image_id'].apply(lambda x: f"COCO_val2014_{x:012}.jpg")
df[['file_name', 'caption']].to_csv(os.path.join(args.output_images_dir, "validation", "metadata.csv"), index=False)
...
with ProcessPoolExecutor(max_workers=args.num_workers) as executor:
    for i, row in df.iterrows():
        ...
        output_img = os.path.join(args.output_images_dir, "validation", image_fname)
